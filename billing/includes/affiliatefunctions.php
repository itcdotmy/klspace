...
function affiliateActivate( $userid )
{
	...
	if( !$affiliateid )
	{
		$affiliateid = insert_query( "tblaffiliates", array(
			"date" => "now()",
			"clientid" => $userid,
			"balance" => $bonusdeposit
		) );
	}
	logActivity( "Activated Affiliate Account - Affiliate ID: {$affiliateid} - User ID: {$userid}", $userid );
	run_hook( "AffiliateActivation", array(
		"affid" => $affiliateid,
		"userid" => $userid
	) );
}
